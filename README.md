...
**Files** shows the project folder as a tree. Click a file to read it; the page cannot change files. It lists the same text files that models can read (no credential files, binary files, files over 200 KB, or folders such as `node_modules`). Use the refresh icon after a model writes new files.
